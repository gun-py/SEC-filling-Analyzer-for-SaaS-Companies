...
PARM_FORMS = ['8-K']
PARM_BGNYEAR = 2021  # User selected bgn period.  Earliest available is 1993 
PARM_ENDYEAR = 2021  # User selected end period.
    ...
    for year in range(PARM_BGNYEAR, PARM_ENDYEAR + 1):
        for qtr in range(PARM_BGNQTR, PARM_ENDQTR + 1):
            ...
            if masterindex:
                ...
                req_cik=[]
                req_orgs=['NICHOLAS FINANCIAL INC']  #change to name of organizations you want 
                for line in masterindex:
                    obj=MasterIndexRecord(line)
                    if obj.name in req_orgs:
                        req_cik.append(line)
                req_cik=masterindex[0:50]
                print('Total number of filings found = ',len(req_cik))
                for line in req_cik:
